...
public class SudokuGrid {
    ...
    public void printRealVals() {
        for(int i = 0; i < 9; i++) {
            for(int j = 0; j < 9; j++) {
                if ((grid[i][j] & MASK_IS_DEFAULT) != 0)  {
                    System.out.print("(" + i +", " + j + ") = ");
                    System.out.println(getGridVal(i, j));
                }
            }
        }
    }
     ...
     * program is represented as a 2-dimensional array in Java.
     * JavaFX 1.1 has no notion of multi-dimensional sequences
     * (arrays), so flatten the grid out to a one-dimensional
     * array of size 81 of type int.  This should map to a JavaFX
